...
```{r, eval=F}
#no need to run unless you haven't run the previous chunk
...
#Note: the datatable command is for incorporating this into the .Rmd file.
#Note: There is often a difference between exploring data in .R or in .Rmd
#files. This is one of the reasons why I tend to build by scripts in .R #files.
pacman::p_load(DT)
...
One common situation to use `replace()` is **changing just one value in one row, using a unique row identifier**. Below, the gender is changed to "Female" in the row where the column `case_id` is "2195".  
